...
local function scroll_hint(search)
    ...
    if search then
        hint = hint .. "，并用 %s 搜索" .. o.key_search
    end
    hint = hint .. ")"
...
local function add_video(s)
    ...
    if o.show_frame_info then
        ...
        local timecodes = {
            ...
            ["estimated-smpte-timecode"] = "估算SMPTE",
        }
...
local function add_track(c, t, i)
    ...
    append(c, t["src-id"], {prefix="解封装ID:", nl="", indent=o.prefix_sep .. o.prefix_sep})
    ...
    if append(c, t["codec-desc"], {prefix="格式:"}) then
    ...
    if t["demux-par"] then
        ...
        append(c, string.format("%d:%d", num, den), {prefix="像素纵横比:"})
    end
    ...
    if t["dolby-vision-profile"] or t["dolby-vision-level"] then
        ...
        append(c, t["dolby-vision-profile"], {prefix="配置档:", nl="", indent=""})
...
local function cache_stats()
    ...
    append(stats, opt_time(a) .. " - " .. opt_time(b), {prefix = "包队列:"})
    ...
    append(stats, info["debug-low-level-seeks"], {prefix = "媒体跳转:"})
    append(stats, info["debug-byte-level-seeks"], {prefix = "流跳转:"})
...
pages = {
    ...
    [o.key_page_4] = { idx = 4, f = keybinding_info, desc = "当前键位", scroll = true },
    [o.key_page_5] = { idx = 5, f = track_info, desc = "音视频轨信息", scroll = true },
